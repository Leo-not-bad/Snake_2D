...
void start(string& string1, float delay = 0.3, float delay_tap = 0.01, short total_elements = 100, short snake_color_over = 255, short snake_color_for_purple = 0, short value_for_show_level = 0)
{
	...
	bool acceleration = false; // ускорение 
	...
	//pointx, pointy - координаты money_pack
	...
	while (window.isOpen())
	{
		...
		// считывание движений и перемещение змеи
		...
		if (acceleration) // ускорение по нажатию
			delay = delay_tap;
	   ...
	   // ускорение и постоянное движение змеи
		...
		// встреча с препятствием и его следующее появление
